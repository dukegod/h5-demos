    ...
    success: function (data) {
      ...
      if (data && data.data) {
        if (sessionStorage.getItem('localDate')) {
          var localDate = sessionStorage.getItem('localDate')
            .replace(/\]$/, "");
          sessionStorage.setItem('localDate', localDate + ',' + JSON.stringify(data.data)
            .replace(/^\[/, ""));
        } else {
        ...
        var adsText = doT.template($("#pageTemplate")
          .text());
        $("#page")
          .append(adsText(data.data));
        flag = true;
...
function readLocal() {
  setTimeout(function () {
    ...
    $("#page")
      .html(adsText(ssh));
    $("#page")
      .scrollTop(parseInt(sessionStorage.getItem('scrollTop_n')));
  }, 20);
}
...
window.onload = function () {
  $('li')
    .on('click', function (e) {
      ...
      sessionStorage.setItem('scrollTop_n', $("body")
        .scrollTop());
    })
