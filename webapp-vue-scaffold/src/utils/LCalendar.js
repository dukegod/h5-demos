    ...
    MobileCalendar.prototype = {
        ...
        bindEvent: function(type) {
            ...
            function popupDate(e) {
                ...
                date_ctrl.addEventListener('touchstart', stopProp);
                var gearDateDiv = _self.gearDate;
                gearDateDiv.addEventListener('touchstart', closeMobileCalendar);
                /**/
            ...
            function gearTouchEnd(e) {
                ...
                target["n_t_1" + target.id] = (new Date()).getTime();
                var flag = (target["new_" + target.id] - target["old_" + target.id]) / (target["n_t_" + target.id] - target["o_t_" + target.id]);
                /*console.log('target1',target["n_t_" + target.id]);
                console.log('target2',target["n_t_1" + target.id]);*/
            ...
            function rollGear(target) {
                ...
                target["int_" + target.id] = setInterval(function() {
                    ...
                    var temp=target["n_t_1" + target.id]-target["n_t_" + target.id]
                    var speed;
                    if(temp>30){
                      speed = target["spd_" + target.id] * Math.exp(-0.9 * d);
                    }else{
                      speed = target["spd_" + target.id] * Math.exp(-0.2 * d);
                    }
                    ...
                    switch (target.dataset.datetype) {
                        case "date_yy":
                            ...
                            break;
                        case "date_mm":
                          /*update by zy 解决活动未终止就关闭弹窗后报错问题*/
                          if(_self.gearDate){
                            var date_yy = _self.gearDate.querySelector(".date_yy");
                            //得到年份的值
                            var yyVal = parseInt(date_yy.getAttribute("val"));
